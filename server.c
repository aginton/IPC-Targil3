...
int createAndEncryptNewPW(Key* out_key, PW* out_plain_pw, PW* out_encrypted_pw);

int createAndEncryptNewPW(Key *key_p, PW *plain_pw_p, PW* out_encrypted_pw)
{
    createPrintablePW(plain_pw_p);
    (plain_pw_p->pw_id)++;
    MTA_get_rand_data(key_p->key, key_p->key_len);

    if (MTA_CRYPT_RET_OK != MTA_encrypt(key_p->key, key_p->key_len, plain_pw_p->pw_data, plain_pw_p->pw_data_len, out_encrypted_pw->pw_data, &(out_encrypted_pw->pw_data_len)))
    {
...
void handleDisconnectRequest(DisconnectReq* disconnect_req_p, bool connected_clients[])
{

}

void handlePWGuess(DecrypterMsg* decrypter_msg_p, PW* plain_pw_p, bool connected_clients[], mqd_t client_mqs[])
{

}

void handleMsg(PW* plain_pw_p, PW* encrypted_pw_p, mqd_t* server_mq_p, bool connected_clients[], mqd_t client_mqs[], bool* out_create_new_pw_flag)
{
    ...
    case CONNECT_REQUEST:
        ...
        handleConnectRequest((ConnectReq*)msg_p->data, encrypted_pw_p, connected_clients, client_mqs);
        break;

    case DISCONNECT_REQUEST:
        //printf("[%s process %d]\t\tserverRespondToMessage() - Received DISCONNECT_REQ message.\n", server_src, getpid());
        handleDisconnectRequest((DisconnectReq*)msg_p->data, connected_clients);
        break;
    ...
    case DECRYPTER_PW_GUESS:
        ...
        handlePWGuess((DecrypterMsg*)msg_p->data, plain_pw_p, connected_clients, client_mqs);
        break;
...
int main()
{
    ...
    if (-1 == (server_mq = mq_open (MQ_SERVER_NAME, O_CREAT | O_EXCL | O_RDONLY | O_NONBLOCK, QUEUE_PERMISSIONS, &attr)))
    {
    ...
    createAndEncryptNewPW(&key, &plain_pw, &encrypted_pw);
    sendClientsEncryptedPW(&encrypted_pw, connected_clients, client_mqs);
    

    bool create_new_pw_flag = false;
    
    while (true)
    {

        // int rc = waitForGuessOrTimeout();
        if (doesMQHaveMessages(&server_mq))
        {
            handleMsg(&plain_pw, &encrypted_pw, &server_mq, connected_clients, client_mqs, &create_new_pw_flag);
        }


        if (create_new_pw_flag)
        {
            createAndEncryptNewPW(&key, &plain_pw, encrypted_pw, &encrypted_pw_len);
            sendClientsEncryptedPW(encrypted_pw, encrypted_pw_len, plain_pw.pw_id);
        }
    }
}